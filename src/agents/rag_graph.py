...
def create_rag_graph(
    ...
    workflow.add_conditional_edges("check_cache", decide_after_cache)
    ...
    workflow.add_conditional_edges(
        "grade", nodes.decide_to_generate, {"web_search": "web_search", "generate": "generate"}
    )
    ...
    workflow.add_conditional_edges(
        ...
        {"verify": "verify", "retry": "increment"},
    )

    # Conditional edge after verification
    workflow.add_conditional_edges(
        "verify", nodes.decide_after_verification, {"end": END, "retry": "increment"}
    )
...
if __name__ == "__main__":
    ...
    # Create a test query
    result = query_rag_agent(question="What is machine learning?", max_iterations=3)
