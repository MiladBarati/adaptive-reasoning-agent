...
cors_origins_str = os.getenv("CORS_ORIGINS", "http://localhost:3000,http://localhost:8000,http://127.0.0.1:3000,http://127.0.0.1:8000")
...
async def query_stream(request: QueryRequest) -> StreamingResponse:
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            for state_update in app_graph.stream(initial_state):
                # LangGraph stream returns dict mapping node names to their output states
                # Get the state from the last node that ran
                if state_update:
                    # Get the state from the last node in the update
...
async def ingest_files(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error ingesting files: {str(e)}")
