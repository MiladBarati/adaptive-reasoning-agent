...
class AdvancedRetriever:
    ...
    def __init__(
        self, vector_store_manager: VectorStoreManager, k: int = 4, use_multi_query: bool = False
    ) -> None:
        """
    ...
    def retrieve(self, query: str, filter: dict[str, Any] | None = None) -> list[Document]:
        ...
        with tracer.start_as_current_span("retriever.retrieve") as span:
            ...
            if self.use_multi_query:
                ...
                docs = self.vector_store_manager.similarity_search(query, k=self.k, filter=filter)
    ...
    def _multi_query_retrieve(
        ...
        for q in query_variations:
            docs = self.vector_store_manager.similarity_search(q, k=self.k, filter=filter)
            for doc in docs:
